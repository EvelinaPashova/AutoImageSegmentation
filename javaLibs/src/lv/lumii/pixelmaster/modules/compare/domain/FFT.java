...
public class FFT {
    ...
    public void perform(float[] real, float[] imag)
    {
        ...
        for (int L = 2; L <= N; L = L+L) {
            for (int k = 0; k < L/2; k++) {
                float kth = -2 * k * (float)Math.PI / L;
                float[] w = new float[2];
                w[0] = (float)Math.cos(kth);
                w[1] = (float)Math.sin(kth);
                //Complex w = new Complex(Math.cos(kth), Math.sin(kth));
                for (int j = 0; j < N/L; j++) {

                    float[] tao = new float[2];

                    tao[0] = (w[0]*real[j*L + k + L/2]) - (w[1] * imag[j*L + k + L/2]);
                    tao[1] = (w[0]*imag[j*L + k + L/2]) + (w[1] * real[j*L + k + L/2]);

                    real[j*L + k + L/2] = real[j*L + k] - tao[0];
                    imag[j*L + k + L/2] = imag[j*L + k] - tao[1];

                    real[j*L + k] = real[j*L + k] + tao[0];
                    imag[j*L + k] = imag[j*L + k] + tao[1];

                    //System.out.println(real[j*L + k + L/2] + ";" + real[j*L + k]);
                }
            }
        }
    }

    public void iperform(float[] real, float[] imag)
    {
        int N = real.length;
